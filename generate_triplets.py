...
def generate_triplets(cd):
    ...
    for image_id in train_img_ids:  # Go through each Train image id
        train += generate_triple_set(image_id, list(set(train_img_ids) - {image_id}), cd)  # Generate and add triplet
    for image_id in val_img_ids:
        validation += generate_triple_set(image_id, list(set(val_img_ids) - {image_id}), cd)
